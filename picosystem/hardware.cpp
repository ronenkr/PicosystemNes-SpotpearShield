...
namespace picosystem {
  ...
  enum pin {
    ...
    AUDIO = 20, RX = 21,                                  // audio //speaker out
  ...
  for(uint32_t i=0;i<0x20000l;i++){//Highly callibrated wait loop...
    asm("nop");
  }
  ...
  void _screen_command(uint8_t c, size_t len = 0, const char *data = NULL) {
    ...
    if(data) {
      ...
      spi_write_blocking(spi1, (const uint8_t*)data, len);
    }
    gpio_put(CS, 1);
  }
